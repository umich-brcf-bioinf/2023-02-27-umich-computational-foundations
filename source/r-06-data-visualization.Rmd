...
As we did with the GDP per capita vs Life expectancy plot above, let's reach into the `gapminder` data and sanity check some of the outliers we see in this plot. It looks like there were two serious dips in life expectancy for two countries. One in 1977 and one in 1992. Let's see if we can figure out what countries those are.

Starting with 1977, we might filter on that year, and then order the life expectancy, looking for the lowest value.

```{r outlier-finding-2}
gapminder %>% filter(year == 1977) %>% arrange(lifeExp)
gapminder %>% filter(year == 1992) %>% arrange(lifeExp)
```

It looks like the minimum life expectancy for 1977 is Cambodia at 31.2 years, and for 1992 is Rwanda at 23.6 years. This highlights something about our plot that perhaps we didn't consider before. The y-axis is not from 0 to 80, but is actually from 20 to 80. Ggplot2 has made a choice about the y-axis limits that might have slipped past our noticing.
...
What if we wanted to plot the global total GDP per year as a bar plot, separated by continent? Let's first take a look at the help function for `geom_bar()`:
...
ggplot(data = gapminder, aes(x = year, y = total_gdp, fill = continent)) +
    geom_col() +
    labs(title = 'Global GDP Over Time (by continent)', x = 'Year', y = 'Total GDP', fill = 'Continent')
```

Let's try to verify that these figures actually make sense. First let's introduce the idea of `position = 'dodge'` where the different categories are plotted side-by-side rather than stacked, as above.

```{r barplot-dodge, purl = FALSE}
p1 = ggplot(data = gapminder, aes(x = year, y = total_gdp, fill = continent)) +
    geom_col(position = 'dodge') +
    labs(title = 'Global GDP Over Time (by continent)', x = 'Year', y = 'Total GDP', fill = 'Continent')
p1
```

This plot is theoretically giving us a sense for aggregate Total GDP by continent per year, but we could also get this data using the `group_by()` and `summarize()` functions.

```{r group-summarize, purl = FALSE}
gdp_by_continent_year = gapminder %>% group_by(continent, year) %>% summarize(aggregate_gdp = sum(total_gdp))
```

In particular, let's look at the Americas and Asia in turn:

```{r america, purl = FALSE}
gdp_by_continent_year %>% filter(continent == 'Americas')
```

Matching these numbers to the plot is a little cumbersome, but we could take advantage of our saved plot object above to quickly add horizontal lines at the values and see if they match. For example, let's plot a horizontal line at 1.22e13 and see if it intersects around 1992, like we would expect.

```{r americas-check, purl = FALSE}
p1 + geom_hline(yintercept = 1.22e13)
```

This doesn't appear to be correct, because the horizontal line intersects our data for the last year, 2007, when according to our summarization, should occur in 1992. So ggplot2 is doing something unexpected here, and it doesn't seem correct.

However, we've already summarized the GDP by continent by year in our `gdp_by_continent_by_year` object, so let's use that as the basis for a plot instead.

```{r barplot-dodge2, purl = FALSE}
p2 = ggplot(data = gdp_by_continent_year, aes(x = year, y = aggregate_gdp, fill = continent)) +
    geom_col(position = 'dodge') +
    labs(title = 'Global GDP Over Time (by continent)', x = 'Year', y = 'Total GDP', fill = 'Continent')
```

And now let's add that horizontal line and see if things make sense now:

```{r barplot-dodge-line, purl = FALSE}
p2 + geom_hline(yintercept = 1.22e13)
```

Indeed, that makes a lot more sense. Moreover, if we look at the data for Asia, we see in 2007 that we just reach over 2 trillion.

```{r asia-subset, purl = FALSE}
gdp_by_continent_year %>% filter(continent == 'Asia')
```

And now if we want to stack the bars again, we can do that:

```{r barplot-stack, purl = FALSE}
ggplot(data = gdp_by_continent_year, aes(x = year, y = aggregate_gdp, fill = continent)) +
    geom_col(position = 'stack') +
    labs(title = 'Global GDP Over Time (by continent)', x = 'Year', y = 'Total GDP', fill = 'Continent')
```

And if we want to make sure we're approaching Global GDP of $6 trillion, we can again, `group_by()` and `summarize()`:

```{r verify-gdp, purl = FALSE}
gdp_by_continent_year %>% group_by(year) %>% summarize(aggregate_yearly_gdp = sum(aggregate_gdp))
```
